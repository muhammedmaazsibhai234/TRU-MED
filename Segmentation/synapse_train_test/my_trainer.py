...
def trainer_synapse(args, model, snapshot_path):
    ...
    exp_lr_scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=400, eta_min=0)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            iter_num, loss.item(), loss_ce.item(), loss_dice.item(), current_lr))
            if iter_num % 20 == 0:
        ...
        loss_total.clear()
        logging.info("epoch loss {} ".format(loss_mean))
        # Test
        ...
        if loss_mean <= loss_best:
            ...
            logging.info("save epoch {} ".format(epoch_num))
        ...
        #     logging.info("*" * 20)
        #     logging.info(f"Test Latest Epoch)")
        #     mean_dice, mean_hd95 = inference(model, testloader, args, test_save_path=test_save_path)
        #     dice_.append(mean_dice)
        #     hd95_.append(mean_hd95)
        #     model.train()
